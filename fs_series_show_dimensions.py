...
def query_video(the_file, width, video_path, i):
    try:
        ...
        vheight_str = str(
            video_streams[0]['coded_height'] if 'coded_height' in video_streams[0] else 0)
        cwidth_str = str(video_streams[0]['width']
                         if 'width' in video_streams[0] else 0)
        cheight_str = str(
        ...
        csv_line = '"{file}",{vwidth_str},{vheight_str},{cwidth_str},{cheight_str}'.format(
            file=video_path,
            vwidth_str=vwidth_str,
            vheight_str=vheight_str,
            cwidth_str=cwidth_str,
...
def transcode_video(filename):
    ...
    command = ['/usr/bin/ffmpeg', '-i', '"' + filename + '"', '-c:v',
               'libx264', '-preset', 'fast', '"' + file_name + '.mkv"']
    #print(' '.join(command))
    p1 = subprocess.Popen(' '.join(command), shell=True,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-d',
        dest='transcode_directory',
        help='folder to transcode'
    )
    parser.add_argument(
        '-f',
