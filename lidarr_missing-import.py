...
def import_track(match): 
    pprint(match)
    response = requests.put(endpoint + "/api/v1/manualimport?", headers={"X-Api-Key":api_key}, json=[match])
    pprint(response.content)
    ...
    if response.status_code == 202:
        ...
        confirm = requests.get(endpoint + "/api/v1/command", headers={"X-Api-Key":api_key}, data=data)
...
def get_album(artistId, albumName):
    response = requests.get(endpoint + "/api/v1/album?artistId=" + str(artistId), headers={"X-Api-Key":api_key})
...
def scan_folder_for_songs(artistId, artistPath, filePath, albumName):
    ...
    response = requests.get(endpoint + "/api/v1/manualimport?" + query, headers={"X-Api-Key":api_key})
    if response.status_code == 200:
        files = response.json()
        for file in files:
    ...
    if response.status_code == 200:
        ...
        for missing in json['records']:
            ...
            if resp.status_code == 200:
                ...
                for track in jsonb:
                    get_song(missing['artist']['id'], missing['artist']['artistName'], missing['title'], track['title'])
