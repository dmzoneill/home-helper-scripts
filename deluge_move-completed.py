...
client = DelugeRPCClient(
    "127.0.0.1", 55555, "deluge", os.environ.get("DELUGE_PASSWORD")
)
client.connect()
torrents_list = client.core.get_torrents_status(
    {}, ["name", "download_location", "label", "progress", "state", "time_added"]
)
...
def remove_series_torrent(id, download_location, name):
    ...
    result = subprocess.Popen(
        " ".join(filebot_cmd), stdout=subprocess.PIPE, shell=True
    ).communicate()[0]
