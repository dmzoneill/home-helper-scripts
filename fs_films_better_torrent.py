...
def get_torrent_listing(name):
    ...
    url = "http://127.0.0.1:9117/api/v2.0/indexers/all/results?apikey=" + jacket_key + "&Query=" + name + "&Category%5B%5D=2000&Category%5B%5D=2040&Category%5B%5D=2045&Tracker%5B%5D=iptorrents&Tracker%5B%5D=privatehd&_=1630824295288"
...
def get_movie(path):
    ...
    onlyvideo = [f for f in onlyfiles if f[-3:] not in ['png', 'srt', 'url', 'nfo', 'jpg', 'den', 'm4v', 'sub', 'idx']]
    ...
    if len(onlyunwanted) == 1 and skip is False:
        if int(os.stat(path + "/" + onlyvideo[0]).st_size) > 733933568 and int(os.stat(path + "/" + onlyvideo[0]).st_size) < 1503933568:
