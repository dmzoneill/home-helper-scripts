...
def get_file_source(file):
    response = requests.get(endpoint + "/logfile/" + file, headers={"X-Api-Key":api_key})
    if response.status_code == 200:
        return response.content.decode('UTF-8').split('\n')
    return []


files = ['Lidarr.txt', 'Lidarr0.txt', 'Lidarr1.txt', 'Lidarr2.txt', 'Lidarr3.txt', 'Lidarr4.txt', 'Lidarr5.txt', 'Lidarr6.txt', 'Lidarr7.txt']
