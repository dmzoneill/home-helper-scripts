    ...
    try:
        ...
        if episode.startswith(series_name + " - " + season_num) is False:
            if "special" in episode.lower():
                if special is True:
                    print("Special: " + series_name + " - " + season_num + " != " + episode)
                return
...
def iterate_series():
    ...
    for rename_from in suggested_series_rename:
        if rename_from != suggested_series_rename[rename_from]:
            print("mv \"" + rename_from + "\" \"" + suggested_series_rename[rename_from] + ".1\"")
            print("mv \"" + suggested_series_rename[rename_from] + ".1\" \"" + suggested_series_rename[rename_from] + "\"")
