...
def set_folder_artist_art(artist_name):
    gio_cmd = (
        'gio set -t unset "' + root + "/" + artist_name + '" metadata::custom-icon'
    )
...
def update_album_artwork(artist, album):
    ...
    try:
        shutil.copyfile(
            path + "/lidarr/MediaCover/Albums/" + str(album["id"]) + "/cover.jpg",
            artist["path"]
            + "/"
            + album["title"]
            + " ("
            + album["releaseDate"][0:4]
            + ")/folder.jpg",
        )
        set_folder_artist_album_art(
            os.fsdecode(artist["artistName"]),
            os.fsdecode(album["title"] + " (" + album["releaseDate"][0:4] + ")"),
        )
...
def update_artist_artwork(artist):
    ...
    try:
        shutil.copyfile(
            path + "/lidarr/MediaCover/" + str(artist["id"]) + "/poster.jpg",
            artist["path"] + "/folder.jpg",
        )
...
def iterate_albums(artist):
    response = requests.get(
        "http://127.0.0.1:8686/api/v1/album?artistid=" + str(artist["id"]),
...
def iterate_artists():
    response = requests.get(
        "http://127.0.0.1:8686/api/v1/artist", headers={"X-Api-Key": api_key}
    )
    if response.status_code == 200:
        json = response.json()
        for artist in json:
