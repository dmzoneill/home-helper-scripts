...
ignored = ["jpg", "hidden", "xattr", "nfo", "idx", "sub", "srt", "ts", "smi"]
...
for series in seriesdirs:
    ...
    # print(series_path)
    seasondirs = sorted(
        [f for f in listdir(series_path) if not isfile(join(series_path, f))]
    )
    ...
    for season in seasondirs:
        ...
        # print(season_path)
        episodes = sorted(
            [f for f in listdir(season_path) if isfile(join(season_path, f))]
        )
        ...
        dupes = [
            item for item, count in collections.Counter(cleaned).items() if count > 1
        ]
